...
class VuerServer:
    def __init__(self, binocular, img_shape, img_shm_name, cert_file="./cert.pem", key_file="./key.pem", ngrok=False, host='0.0.0.0', port=8012):
        ...
        if ngrok:
            self.vuer = Vuer(client="client_pico_teleop",host=host, port=port, queries=dict(grid=False), queue_len=3)
        else:
            self.vuer = Vuer(client="client_pico_teleop",host=host, port=port, cert=cert_file, key=key_file, queries=dict(grid=False), queue_len=3)

        self.vuer.add_handler("HAND_MOVE")(self.on_hand_move)
        self.vuer.add_handler("CAMERA_MOVE")(self.on_cam_move)
        self.vuer.add_handler("PICO_USER_DEFINED_EVENT")(self.on_pico_user_defined_event)
...
if __name__ == '__main__':
    ...
    # television
    tv = VuerServer(False, img_shape, img_shm.name)
